...
export function createPlayer(camera, scene) {
  ...
  const joystick = document.querySelector('#joystick'), controls = document.querySelector('#controls');
  ...
  controls.addEventListener('pointerdown', e => { if (e.target === joystick || joystick.contains(e.target)) return; lookId=e.pointerId; lastX=e.clientX; lastY=e.clientY; controls.setPointerCapture(e.pointerId); });
  controls.addEventListener('pointermove', e => { if (e.pointerId !== lookId) return; state.yaw -= (e.clientX-lastX)*.006; state.pitch -= (e.clientY-lastY)*.004; lastX=e.clientX; lastY=e.clientY; });
  const releaseLook=e=>{if(!e || e.pointerId===lookId) lookId=null}; controls.addEventListener('pointerup',releaseLook); controls.addEventListener('pointercancel',releaseLook);
