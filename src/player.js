...
export function createPlayer(camera, scene) {
  ...
  const joystick = document.querySelector('#joystick'), lookRegion = document.querySelector('#look-region');
  ...
  lookRegion.addEventListener('pointerdown', e => { lookId=e.pointerId; lastX=e.clientX; lastY=e.clientY; lookRegion.setPointerCapture(e.pointerId); e.preventDefault(); });
  lookRegion.addEventListener('pointermove', e => { if (e.pointerId !== lookId) return; state.yaw -= (e.clientX-lastX)*.006; state.pitch -= (e.clientY-lastY)*.004; lastX=e.clientX; lastY=e.clientY; });
  const releaseLook=e=>{if(!e || e.pointerId===lookId) lookId=null}; lookRegion.addEventListener('pointerup',releaseLook); lookRegion.addEventListener('pointercancel',releaseLook);
